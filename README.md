...
AI 应用的用户反馈同时包含通用体验问题和 AI 特有体验问题。评论中既会出现卡顿、入口难找、限制说明不清楚，也会出现回答不准、幻觉、上下文记忆差、指令跟随差、搜索可信度不足、生成质量不稳定等信号。
...
| 归档样本 | 5 条千问相关样本已归档到 `data/excluded_reviews.csv` |

样本采集方式为手动公开采样，数据处理过程保留评论正文、评分、日期、来源等分析所需字段。初始备选应用千问的公开评论中有效 AI 产品体验反馈较少，相关样本已归档，主分析样本聚焦豆包、DeepSeek、Kimi。
...
本项目以 `issue_category` 作为主分析层级，保持第一版样本的大类分析口径。
...
`不明确/其他` 在本项目中作为第一版样本的保守标签使用，维持现有大类口径。正向能力样本单独作为用户期待和产品机会证据。
...
- 当前样本量为 30 条有效评论，适合用于展示分析方法和初步观察。
- 公开应用商店评论可能偏向强情绪用户、近期版本用户和低评分用户。
- 人工编码存在主观性，尤其是复合问题评论需要选择主要问题大类。
- 当前分析使用公开评论数据，产品建议属于基于外部反馈的初步方案；内部埋点、留存、转化和任务完成率可作为后续验证数据。
